<?php 
	...
	if(false){
		...
		$date = date("Y-m-d");
		// Jumlah Loket
		$results = $mysqli->query('SELECT  count(*) as jumlah_loket, nama_dokter FROM client_antrian ');	
		$loket = $results->fetch_array();
		$data['nama_dokter'] = $loket['nama_dokter'];
		$data['jumlah_loket'] = $loket['jumlah_loket']; // set jumlah loket
		...
		if ($count){
			...
			if($rows['id']!=NULL)
			{
				...
				$_SESSION["next_server"][$rows['counter']] = $rows['id'];
				$_SESSION["counter_server"][$rows['counter']] = $rows['counter'];
				$mysqli->query('UPDATE data_antrian SET status= 1 WHERE id='. $rows['id'] .'');
